...
export const useHandlePositions = (
    ...
    // Rebuild index when viewport changes (on end to avoid continuous renders)
    useOnViewportChange({
        onEnd: () => {
            if (enabled) {
                rebuildIndex();
...
export const useViewportHandlePositions = (
    ...
    // We only need to trigger a re-render when viewport pan/zoom ends
    const [, setViewportCounter] = useState(0);

    useOnViewportChange({
        onEnd: () => setViewportCounter(c => c + 1)
    });
    ...
    }, [spatialIndex, canvasSize]); // viewport is deliberately excluded, it triggers via useOnViewportChange
