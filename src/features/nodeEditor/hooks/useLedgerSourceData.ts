...
export const useLedgerSourceData = (
    ...
    const stats = useMemo(() => {
        ...
        fieldIds.forEach(fieldId => {
            // ⚡ Bolt Optimization: Replace chained .map().filter().reduce() and Math.min/max(...values) with a single-pass for loop. Expected impact: ~20x faster execution and reduced memory overhead for large arrays, preventing 'Maximum call stack size exceeded' errors.
            let sum = 0;
            let min = Infinity;
            let max = -Infinity;
            let count = 0;
