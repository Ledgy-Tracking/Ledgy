...
export const useLedgerSourceData = (
    ...
    const stats = useMemo(() => {
        ...
        fieldIds.forEach(fieldId => {
            // ⚡ Bolt: Replaced .map().filter().reduce() and Math.min/max spread with single-pass for-loop
            // Reduces intermediate allocations (O(N) to O(1) space) and avoids call stack exceptions
            let sum = 0;
            let min = Infinity;
            let max = -Infinity;
            let count = 0;
