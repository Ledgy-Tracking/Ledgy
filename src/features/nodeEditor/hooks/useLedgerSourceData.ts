...
export const useLedgerSourceData = (
    ...
    const stats = useMemo(() => {
        ...
        fieldIds.forEach(fieldId => {
            // Bolt: Replaced chained .map().filter() and Math.min/max with a single-pass loop
            // to avoid intermediate array memory allocations and Maximum Call Stack Exceeded
            // errors when processing large ledgers.
            let sum = 0;
            let min = Infinity;
            let max = -Infinity;
            let count = 0;
...
export const useFieldStats = (
    ...
    return useMemo(() => {
        ...
        // Bolt: Single-pass iteration to calculate stats to prevent large array memory allocations
        // and avoid call stack limits associated with Math.min(...values)
        let sum = 0;
        let min = Infinity;
        let max = -Infinity;
        let count = 0;
