...
export const calculateBoundingBox = (nodes: Node[]): ContainerBounds => {
    // ⚡ Bolt: Single-pass loop replaces Map/Spread to avoid Maximum call stack size exceeded
    if (nodes.length === 0) {
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        // Native Math.min/max parity: propagate NaN
        if (Number.isNaN(x) || Number.isNaN(y)) {
            return { minX: NaN, minY: NaN, maxX: NaN, maxY: NaN, width: NaN, height: NaN };
        }
