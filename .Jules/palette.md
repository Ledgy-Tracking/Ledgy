...
## 2024-05-30 - Dynamic aria-expanded for Layout Toggles
**Learning:** For application shell layout toggles (like sidebars and inspector rails), an `aria-label` alone isn't enough. Screen readers need to know the current state (expanded vs collapsed) to provide meaningful context to the user.
**Action:** Always dynamically bind `aria-expanded={isOpen}` to layout toggle buttons rather than hardcoding them to "false" or omitting them entirely.
