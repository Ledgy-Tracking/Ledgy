...
## 2024-06-25 - Static Accessible Names for Toggles
**Learning:** Dynamically toggling `aria-label` strings (e.g., "Open Sidebar" -> "Close Sidebar") for collapsible panels can cause the underlying accessible name to flip unpredictably during navigation, potentially disorienting screen reader users.
**Action:** Always use a static, persistent `aria-label` (e.g., "Toggle sidebar") paired with boolean state attributes (`aria-expanded={isOpen}` or `aria-pressed={isActive}`) to cleanly convey the component's state.
