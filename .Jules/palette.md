...
## 2025-05-18 - ARIA Expanded and Pressed for AppShell Toggles
**Learning:** Icon-only view-switcher buttons and panel toggles in the main AppShell lack `aria-expanded` and `aria-pressed` attributes. This prevents screen reader users from understanding the current state of panels (left sidebar, right inspector) or view modes (table vs grid, dark vs light).
**Action:** Consistently apply `aria-expanded` to buttons that toggle visibility of panels, and `aria-pressed` to buttons that toggle between two distinct states (like view modes or themes).
