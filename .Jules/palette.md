...
## 2024-06-18 - Replacing Missing Dependencies
**Learning:** Build failures due to missing dependencies like `nanoid` (when accidentally used without being explicitly installed) can sometimes be quickly remedied by switching to already-installed identical alternatives, like `uuid` which provides similar ID generation semantics without bloating the application dependencies.
**Action:** When facing a missing UI dependency, prioritize finding an equivalent module already declared in `package.json` to reduce scope creep and comply with boundary directives.
