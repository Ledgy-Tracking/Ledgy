...
## 2025-02-23 - ARIA States for Dashboard Sidebars/Inspectors
**Learning:** For layout toggle buttons that expand or collapse sections (such as sidebars or inspector panels), dynamically binding `aria-label`s like "Open sidebar" vs "Close sidebar" does not convey state well.
**Action:** Replace dynamic `aria-label` values on layout toggles with static labels (e.g., "Toggle sidebar") and dynamically bind the `aria-expanded` attribute to the boolean visibility state of the target panel to ensure the state is correctly communicated to screen readers.
