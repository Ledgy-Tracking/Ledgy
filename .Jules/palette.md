...
## 2025-02-23 - ARIA Attributes for UI Shell Toggle Buttons
**Learning:** Collapsible panels and app shell view-toggle buttons (e.g., Theme, Layout Mode) inherently function as boolean state controllers. For screen reader users, visually highlighting a toggled button is invisible. We learned that `aria-controls` bindings with matching container `id`s are essential for structural collapsibles (like left/right sidebars), while `aria-pressed` correctly represents binary state for visual layout toggles.
**Action:** Always complement `aria-expanded` and `aria-controls` on structural sidebar toggles with their respective `id`s on the structural containers, and use `aria-pressed` for generic boolean feature toggles (like Dark Mode or Grid View).
