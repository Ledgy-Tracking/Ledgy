...
## 2024-05-18 - Communicating Toggle States with ARIA
**Learning:** For layout toggle buttons that show or hide sections (e.g., sidebars, inspector panels), screen readers need to know the state of the panel being toggled. Just having `aria-label` is insufficient.
**Action:** Explicitly add and dynamically bind the `aria-expanded` attribute to the boolean visibility state of the target panel (e.g., `aria-expanded={isOpen}`) to communicate the expanded/collapsed state to screen readers.
