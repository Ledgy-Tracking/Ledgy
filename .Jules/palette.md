...
## 2025-02-23 - ARIA States for AppShell Toggles
**Learning:** Core layout header toggles (sidebar, inspector, theme, view mode) had `aria-label` but lacked `aria-expanded`/`aria-pressed` state attributes. Without these, screen reader users only know what the button does, not its current active mode or layout visibility.
**Action:** Always pair `aria-label` with `aria-expanded` (for collapsible panels) or `aria-pressed` (for binary mode switches) on global shell controls.
