...
## 2025-02-23 - Expanding Structural Navigation Toggles
**Learning:** Collapsible structural elements like the left Sidebar and right Inspector Rail use toggle buttons that dictate the visibility of their containers. While an `aria-label` provides a name, screen reader users also need to know the state of the collapsible area and which area the toggle controls.
**Action:** Always add `aria-expanded={isOpen}` and `aria-controls="container-id"` (along with a corresponding `id="container-id"` on the container) to buttons that toggle structural sidebars or panels to explicitly announce their open/closed state.
