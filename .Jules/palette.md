...
## 2024-06-12 - Static ARIA labels for AppShell toggles
**Learning:** Dynamic aria-labels (e.g. 'Open/Close') on toggle buttons can be confusing for screen readers, and replacing them with static labels + aria-expanded or aria-pressed improves accessibility. The tooltip should also match the static label exactly.
**Action:** Use static aria-labels paired with aria-expanded (for layout sections) or aria-pressed (for state/view switchers) on icon-only toggle buttons. Ensure corresponding tests searching by name are updated.
