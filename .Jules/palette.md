...
## 2025-02-23 - Avoiding Dynamic ARIA Labels with ARIA Pressed
**Learning:** When using `aria-pressed` on a toggle button, combining it with an `aria-label` that dynamically changes based on the state (e.g., "Switch to light mode" vs "Switch to dark mode") is an accessibility anti-pattern. Screen readers may announce both the dynamic action and the pressed state, confusing users.
**Action:** Always use a static `aria-label` (e.g., "Toggle dark mode") when implementing `aria-pressed` on toggle buttons to ensure clean and predictable screen reader announcements.
