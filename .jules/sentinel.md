...
## 2024-05-20 - [ReDoS in Dynamic Schema Generation]
**Vulnerability:** The application allowed users to specify arbitrary regular expressions for schema validation, which were directly compiled into `RegExp` objects without length or complexity checks. This enabled a Regular Expression Denial of Service (ReDoS) vulnerability where complex nested quantifiers (e.g., `(a+)+`) could be used to freeze the client UI during data entry validation.
**Learning:** In a local-first application where schema definitions are dynamically driven by users, even client-side validations must treat user-provided regex patterns as untrusted execution logic, not just data.
**Prevention:** Always validate user-provided regular expressions for dangerous nested quantifiers and enforce strict maximum length limits before compiling them into RegExp instances or passing them to validation libraries like Zod.
