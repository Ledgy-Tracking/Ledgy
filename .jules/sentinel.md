...
## 2026-03-25 - Insecure Random Generation for Identifiers (Canvas Nodes)
**Vulnerability:** The application used `Math.random()` as a fallback for generating action IDs in `useUndoRedoStore.ts` and also for node IDs in `NodeCanvas.tsx`. This non-cryptographic generator can produce predictable outputs, potentially allowing an attacker to guess identifiers or bypass security checks relying on ID uniqueness.
**Learning:** `Math.random()` is not cryptographically secure and should never be used for generating identifiers or tokens. `crypto.randomUUID()` may not be available in all contexts (e.g., insecure contexts), so a robust, cryptographically secure fallback like the `uuid` package (`uuidv4()`) must be used instead of manual generation using `Math.random()`.
**Prevention:** Always use `crypto.randomUUID()` or a well-vetted library like `uuid` for unique identifiers to guarantee cryptographic security and unpredictability, and avoid manual fallback implementations relying on `Math.random()`.
