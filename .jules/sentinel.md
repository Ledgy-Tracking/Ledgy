...
## 2024-05-26 - ReDoS via Unbounded Regex Patterns
**Vulnerability:** The schema builder permitted users to input unrestricted JavaScript regular expressions (via `new RegExp(field.pattern)`). Malicious or poorly constructed regexes with nested unbounded quantifiers (like `(a+)+`) could cause catastrophic backtracking (ReDoS) during validation, freezing or crashing the client application.
**Learning:** Client-side form builders that allow custom regex input are highly susceptible to ReDoS. Trusting user-provided regex patterns directly in `new RegExp()` without validation or length boundaries is a critical vulnerability.
**Prevention:** Always sanitize and limit custom regex patterns. Implement length restrictions (e.g., max 250 characters) and use heuristics (e.g., detecting `(?:[+*]|\{\d+(?:,\d*)?\})` inside unescaped groups) to block obviously dangerous nested quantifiers before compiling them.
