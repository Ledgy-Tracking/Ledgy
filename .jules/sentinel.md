...
## 2026-04-15 - Regular Expression Denial of Service (ReDoS) via User Input
**Vulnerability:** The `SchemaBuilder` and `validation.ts` modules allowed arbitrary user-provided strings to be compiled into `RegExp` objects (`new RegExp(pattern)`) and used for form validation (e.g. within Zod schemas) without checking the pattern's length or structure. Maliciously crafted regex patterns with nested quantifiers (e.g., `(a+)+`) could cause catastrophic backtracking, freezing the thread (client-side DoS) when evaluated against specific inputs.
**Learning:** Never pass unsanitized user input directly into `new RegExp()`. User-provided regular expressions can contain catastrophic backtracking traps that consume exponential time and freeze the JavaScript runtime.
**Prevention:** All user-provided regex strings must be validated before compilation. Enforce length constraints (e.g., 250 characters) and use heuristics to detect and reject dangerous patterns like nested quantifiers (e.g., checking for `(...)+` constructs) before allowing the string to be used as a regex.
