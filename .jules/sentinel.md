...
## 2026-06-03 - RegExp ReDOS Vulnerability
**Vulnerability:** The application was directly compiling user-provided RegExp pattern strings from `SchemaBuilder.tsx` and processing them in Zod validations (`src/lib/validation.ts`). An attacker could provide a regex pattern with dangerous nested quantifiers (e.g. `(a+)+`) or excessively long patterns, causing the JavaScript engine to hang or consume excessive CPU resources (ReDOS - Regular Expression Denial of Service).
**Learning:** Any user-provided RegExp pattern string should be rigorously validated before compilation to avoid client-side denial of service attacks. We cannot trust user-provided regexes inherently, even if we expect developers/admins to be the primary users configuring schemas.
**Prevention:** Always validate regex patterns using `validateRegexPattern` from `src/utils/security.ts` to enforce length limits and heuristic checks against known ReDoS anti-patterns (like nested quantifiers) before invoking `new RegExp()` or passing them into tools like Zod.
