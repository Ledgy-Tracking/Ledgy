...
## 2024-05-28 - ReDoS Vulnerability in Ledger Schema Pattern
**Vulnerability:** A Regular Expression Denial of Service (ReDoS) vulnerability existed in `SchemaBuilder.tsx` and `validation.ts` where unvalidated user input was passed directly to the `RegExp` constructor.
**Learning:** The application allowed users to define schema fields with arbitrary regex patterns without bounding length or checking for catastrophic backtracking patterns (like nested quantifiers `(a+)+`).
**Prevention:** Always sanitize and validate user-supplied regex strings before compiling them. Introduced `validateRegexPattern` in `src/utils/security.ts` to enforce length limits and use heuristics against known dangerous ReDoS patterns.
