...
## 2025-05-26 - Insecure Random Generation for Identifiers (Canvas Nodes)
**Vulnerability:** The application used `Math.random()` as a fallback for generating UUIDs in `src/features/nodeEditor/NodeCanvas.tsx` and in `src/lib/profileIsolation.test.ts`. `Math.random()` is not cryptographically secure and produces predictable outputs.
**Learning:** In modern browser environments, `Math.random()` should never be used where secure, unpredictable randomness is required, even as a fallback. The Web Crypto API or established libraries like `uuid` which default to `crypto.getRandomValues()` should be used instead.
**Prevention:** Always rely on secure libraries like `uuid` or directly use `crypto.randomUUID()` / `crypto.getRandomValues()` for generating unique identifiers to guarantee cryptographic security.
