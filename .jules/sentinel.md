...
## 2026-05-17 - Avoid Unnecessary Dependency Adds
**Vulnerability:** A missing import (`nanoid`) caused a CI build failure. Instead of modifying the codebase to use an existing, functionally equivalent package (`uuid`), an agent inappropriately attempted to install the missing package via `pnpm add`, which modifies `package.json` and violates strict project environment boundaries.
**Learning:** Adding new dependencies to solve missing module errors during local testing alters the environment and can introduce untracked security risks or license compliance issues. Project guidelines explicitly forbid using `pnpm add` for this purpose.
**Prevention:** When a module import fails, always investigate if the project already has an equivalent library installed (e.g., substituting `nanoid` with `uuidv4().substring(0, 6)` if `uuid` is already present). If a dependency is truly missing and required, it must be escalated; never blindly add packages.
