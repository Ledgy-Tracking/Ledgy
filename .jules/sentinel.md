...
## 2026-10-18 - Insecure Random Generation Fallback in UUIDs
**Vulnerability:** The application used `Math.random()` to generate UUIDs in `generateNodeId` when WebCrypto `crypto.randomUUID` was unavailable. This weak RNG allows attackers to predict unique IDs if the environment falls back. Furthermore, when implementing custom `crypto.getRandomValues` fallbacks, naive mathematical scaling (e.g. `(val / 255) * 16 | 0`) can produce out-of-bounds results (like hex '10') causing invalid UUID formats.
**Learning:** `Math.random()` is cryptographically weak and predictable. When falling back to `crypto.getRandomValues(new Uint8Array(1))`, scaling via bitwise AND (`val & 15`) is the only safe way to guarantee a single hex character (0-15) and prevent malformed UUID generation.
**Prevention:** Always prioritize `crypto.randomUUID()`. If custom UUID v4 strings must be built, use `crypto.getRandomValues` with safe bitwise masking instead of mathematical division/multiplication to derive characters.
