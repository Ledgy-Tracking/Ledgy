...
## 2025-02-14 - Replace weak Math.random() with secure UUID generation
**Vulnerability:** Found multiple instances where `Math.random()` was used for unique ID generation, specifically as a manual fallback for UUID v4 generation in `NodeCanvas.tsx` and for test ID generation in `hydration.integration.test.ts` and `profileIsolation.test.ts`. This poses a security risk as `Math.random()` is not cryptographically secure and predictable, which could lead to ID collisions and predictable values in non-HTTPS contexts.
**Learning:** Manual fallback implementations for unique IDs often rely on weak randomness if secure APIs (like `crypto.randomUUID()`) are unavailable. Additionally, SAST tools flag `Math.random()` usage for ID generation even in tests, requiring a secure alternative across the board. The `uuid` library provides a robust, cross-environment fallback out of the box using `crypto.getRandomValues()`.
**Prevention:** Always standardize on a cryptographic generator like `uuidv4()` from the `uuid` package for all ID generation tasks, rather than writing custom fallbacks or using `Math.random()`, to ensure security and pass static analysis checks.
