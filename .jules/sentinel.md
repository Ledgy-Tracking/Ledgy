...
## 2024-05-29 - Regular Expression Denial of Service (ReDoS) via User Input
**Vulnerability:** The application allowed users to provide arbitrary string patterns for regular expression validation in `SchemaBuilder.tsx`. These patterns were compiled using `new RegExp()` without any length limits or complexity checks. A malicious user could input a string with nested or overlapping quantifiers (e.g., `(a+)+`), which takes exponential time to evaluate, causing a ReDoS vulnerability that crashes or freezes the client-side JavaScript engine.
**Learning:** Instantiating regular expressions directly from unsanitized user input is extremely dangerous. The complexity of regular expression evaluation can easily be manipulated to cause denial of service. Standard regex engines (like the one built into JavaScript) are vulnerable to catastrophic backtracking.
**Prevention:** Always validate user-provided regular expressions using a combination of maximum length limits (e.g., 250 characters) and heuristic checks to reject dangerous patterns (such as nested quantifiers) before attempting to compile or execute them. Utilize a centralized utility like `validateRegexPattern` for this purpose.
