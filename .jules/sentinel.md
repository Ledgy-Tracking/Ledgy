...
## 2026-05-20 - Insecure Randomness in UUID Fallbacks
**Vulnerability:** The `generateNodeId` fallback logic in `NodeCanvas.tsx` used `Math.random()` to generate UUIDs when `crypto.randomUUID` was unavailable (e.g., in non-HTTPS contexts). `Math.random()` is not cryptographically secure, leading to predictable node identifiers.
**Learning:** Custom fallback implementations for UUID generation often rely on insecure randomness. Even for non-security critical identifiers, predictability can lead to collisions or enumeration. Modern environments should rely on standard libraries (`uuid`) that correctly leverage `crypto.getRandomValues()`.
**Prevention:** Always use established libraries (like `uuid`) for unique identifier generation. Avoid hand-rolled `Math.random()` based fallback implementations.
