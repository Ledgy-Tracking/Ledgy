...
## 2026-03-24 - Math.random() as Insecure Random Generation
**Vulnerability:** The application used `Math.random()` as a fallback for generating action IDs in `NodeCanvas.tsx` (used for UUIDs) and in various testing files for document IDs and test suffixes. While less critical in test files, using `Math.random()` in `NodeCanvas.tsx` to generate unique identifiers can produce predictable outputs, potentially leading to collision issues or allowing attackers to guess identifiers.
**Learning:** `Math.random()` is not cryptographically secure and its use as a fallback for UUID generation is an anti-pattern when standard libraries like `uuid` are available and robust.
**Prevention:** Always rely on well-tested, standard libraries like `uuid` (e.g., `uuidv4()`) for generating unique identifiers, which correctly handle environments without `crypto.randomUUID()` securely.
