...
## 2024-05-24 - ReDoS in Schema Builder
**Vulnerability:** A Regular Expression Denial of Service (ReDoS) vulnerability was found in `SchemaBuilder.tsx` and `validation.ts`. The application allowed arbitrary user-provided RegExp strings (e.g., from schema pattern definitions) to be compiled using `new RegExp()` without proper validation.
**Learning:** This exposes the application to DoS attacks where an attacker can craft a complex, nested regex pattern that takes exponential time to evaluate, hanging the client or server.
**Prevention:** Always validate user-provided regular expressions using a dedicated utility (like `validateRegexPattern`) to enforce length limits and use heuristics to reject dangerous nested quantifiers before passing them to `new RegExp()`.
