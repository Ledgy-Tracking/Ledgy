...
## 2024-05-21 - ReDoS Prevention in Schema Builder

**Vulnerability:** The Schema Builder allowed users to input custom regular expressions for field validation. These regex strings were directly passed to `new RegExp()` and used in Zod validation without any checks for patterns that could cause Regular Expression Denial of Service (ReDoS).
**Learning:** User-provided regex patterns are inherently dangerous because they can contain nested quantifiers or large repetitions that cause the regex engine to backtrack exponentially, consuming excessive CPU and memory (ReDoS), which can crash the application or slow it down significantly.
**Prevention:** Always validate user-provided regex patterns using a utility function (like `validateRegexPattern` from `src/utils/security.ts`) that limits the pattern length and checks for dangerous constructs (e.g., nested quantifiers `(a+)+` or large repetitions `{1000}`) before compiling them into a `RegExp` object or using them in validation libraries like Zod.
