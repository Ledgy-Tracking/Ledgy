...
## 2025-05-26 - Regular Expression Denial of Service (ReDoS)
**Vulnerability:** The application allowed users to define arbitrary regular expressions in `SchemaBuilder.tsx` and applied them directly to schema validation in `validation.ts` via `zod`. This could allow a malicious user to craft a complex, nested quantifier regex (e.g., `(a+)+`) that would cause catastrophic backtracking, hanging the JavaScript engine and causing a client-side Denial of Service.
**Learning:** All user-provided regular expressions must be strictly validated before compilation or evaluation. A simple syntax check (`try { new RegExp() }`) is insufficient as it does not evaluate execution complexity.
**Prevention:** Implement a dedicated validation utility (`validateRegexPattern`) that enforces strict length limits (e.g., 100 characters) and uses heuristic checks to block known dangerous patterns (like nested quantifiers) before allowing the regex to be compiled or applied to user data.
