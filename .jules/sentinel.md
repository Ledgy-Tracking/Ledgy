...
## 2025-03-01 - ReDoS vulnerability in user-defined schema regex
**Vulnerability:** Unbounded user-defined regexes were compiled and used in Zod validation logic (), which is susceptible to Regular Expression Denial of Service (ReDoS) attacks when a user creates patterns with nested quantifiers (e.g. `(a+)+`).
**Learning:** In a dynamic schema builder where users define their own validation constraints, directly executing untrusted regex strings without validation or sanitization exposes the application to catastrophic backtracking during validation.
**Prevention:** Apply strict limits (e.g., maximum length) and heuristics to block dangerous regex patterns (like nested quantifiers) before they are passed to the  constructor or Zod's  method.
## 2025-03-01 - ReDoS vulnerability in user-defined schema regex
**Vulnerability:** Unbounded user-defined regexes were compiled and used in Zod validation logic (`new RegExp(field.pattern)`), which is susceptible to Regular Expression Denial of Service (ReDoS) attacks when a user creates patterns with nested quantifiers (e.g. `(a+)+`).
**Learning:** In a dynamic schema builder where users define their own validation constraints, directly executing untrusted regex strings without validation or sanitization exposes the application to catastrophic backtracking during validation.
**Prevention:** Apply strict limits (e.g., maximum length) and heuristics to block dangerous regex patterns (like nested quantifiers) before they are passed to the `RegExp` constructor or Zod's `.regex()` method.
