...
## 2026-04-20 - Secure Identifier Generation Fix
**Vulnerability:** Use of `Math.random()` for generating UUIDs in `src/features/nodeEditor/NodeCanvas.tsx` fallback and `testSuffix` in `src/lib/profileIsolation.test.ts`.
**Learning:** Although often used as a quick workaround for environments without `crypto.randomUUID()`, `Math.random()` is cryptographically insecure and prone to collisions and predictability. This presents a security risk, particularly in generating identifiers.
**Prevention:** Always rely on secure cryptographic libraries, such as `uuid` (e.g. `uuidv4()`) or the Web Crypto API (`crypto.randomUUID()` or `crypto.getRandomValues()`) for random value and ID generation.
