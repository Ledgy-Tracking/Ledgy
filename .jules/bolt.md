...
## 2024-05-28 - Optimize array map/filter chains and spread operator for Math min/max
**Learning:** Chained `.map().filter()` over large dataset extracts creates intermediate array allocations. Furthermore, extracting array values and spreading them using `Math.max(...values)` and `Math.min(...values)` can trigger a 'RangeError: Maximum call stack size exceeded' on very large datasets (>100k items) because V8 treats the spread arguments as individual function arguments.
**Action:** Consistently replace these patterns in aggregation loops with a manual, single-pass `for` loop to compute stats (sum, min, max, count) simultaneously. This prevents the call stack limit exception while also avoiding memory allocation overhead from intermediate mapped/filtered arrays.
