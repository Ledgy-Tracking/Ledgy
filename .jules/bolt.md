...
## 2026-04-17 - Array.prototype.reduce and single-pass iteration optimization
**Learning:** In hot loops and statistical calculations, using `.reduce()` incurs function call overhead and implicit intermediate memory allocations. In addition, array operations like `.map().filter()` create temporary intermediate array instances that must be garbage collected, increasing GC pressure and O(N) operations.
**Action:** Use single-pass `for` loops with scalar accumulation variables to optimize data reductions, especially when chaining transformations.
