...
## 2024-05-28 - Avoid chained filter and map for node array aggregations
**Learning:** Chaining `.filter(condition).map(extraction)` on potentially large data structures like node arrays (e.g., in `getAllDescendants` or `isInternalConnection`) forces JavaScript to allocate intermediate arrays and traverse the dataset multiple times, leading to O(N) memory overhead and unnecessary CPU cycles per call. In highly dynamic paths like connection validation, this adds up quickly.
**Action:** Always replace chained `.filter().map()` array operations with a single-pass traditional `for` loop that manually iterates the array and conditionally pushes/adds values to the accumulator (array or Set).
