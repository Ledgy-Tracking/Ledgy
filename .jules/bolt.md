...
## 2024-05-28 - Optimized node graph validation loop
**Learning:** In the `validateContainerIntegrity` function, running iterative repair checks that use `.find()`, `.filter()`, and `.findIndex()` against the entire `nodes` array inside loops over that same array resulted in severe O(N^2) complexity. This caused noticeable performance degradation during diagram validation as the number of nodes scaled. We also learned that placing `new Map(...)` allocations inside a `.map()` callback (which processes every node) is an anti-pattern that creates excessive GC pressure and negates some of the lookup benefits.
**Action:** When performing multi-pass validation on a large collection (like ReactFlow nodes), always build O(1) hash maps (`Map`) at the top of the function for lookups (e.g., `nodesById`, `childrenByParent`, `repairedIndexMap`). Crucially, ensure that these `Map` instances are instantiated *outside* of any looping/mapping callbacks so they are only allocated once.
