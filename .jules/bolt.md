...
## 2024-05-28 - Optimize array reduction operations with loops
**Learning:** In statistical processing modules like `utils/statistics.ts`, chaining `Array.prototype.reduce()` and spread operations like `Math.min(...values)` can introduce significant function call overhead and "Maximum call stack size exceeded" errors on extremely large arrays.
**Action:** Replace `reduce()` with explicit `for` loops to minimize callback overhead. Replace spread operators in `Math.min` and `Math.max` with manual loop iterations. When performing Pearson correlation, consolidate multiple reduction passes (computing means, then variances) into a single optimized pass computing sums and sums of squares, reducing Big-O iterations from 3N to N. Ensure handling of `NaN` mimics native math behavior (e.g., returning NaN when encountered).
