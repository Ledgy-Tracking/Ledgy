...
## 2024-05-30 - Replace array mapping with single-pass for loop
**Learning:** In data processing pipelines and state hooks, chaining array methods like `.map().filter()` combined with `Math.max(...values)` creates redundant iterations and places elements on the call stack, leading to `RangeError: Maximum call stack size exceeded` for large datasets.
**Action:** Replace chained `.map().filter()` and spread arguments with a single-pass `for` loop to accumulate minimums, maximums, sums, and counts to avoid intermediate allocations and call stack limitations.
