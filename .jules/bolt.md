...
## 2024-05-27 - Avoid Array Spread with Math.max/min
**Learning:** Using `Math.max(...xs.map(...))` on large arrays can cause `RangeError: Maximum call stack size exceeded` because the spread operator places array elements on the call stack. It also causes unnecessary garbage collection due to intermediate array allocations via `.map()`.
**Action:** When finding the min/max of large arrays or computing bounds, avoid `.map()` and spread syntax. Instead, use a single-pass `for` loop to accumulate the `min` and `max` values explicitly. This approach is O(n) in time, O(1) in space, and completely avoids call stack limitations.
