...
## 2024-05-28 - Optimize array diffs in Zustand subscriptions
**Learning:** Using `Array.prototype.find` inside `Array.prototype.filter` to diff arrays in a Zustand store subscription (like `useNodeStore.subscribe`) results in $O(N^2)$ complexity, causing severe performance issues on high-frequency state updates as $N$ grows.
**Action:** Always map the previous array elements into a `Map` keyed by their IDs before the loop. This reduces lookup time to $O(1)$, converting the operation from $O(N^2)$ to $O(N)$.
