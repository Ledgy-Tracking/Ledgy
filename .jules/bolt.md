...
## 2024-11-20 - Array Processing Bottlenecks

**Learning:** When aggregating statistics (min, max, average) over large arrays, chaining `.map().filter().reduce()` alongside `Math.min(...values)` creates massive garbage collection overhead and can trigger 'Maximum call stack size exceeded' errors when `values` is large, due to spread operator limitations.
**Action:** Always prefer a single-pass `for` loop for high-frequency or large-scale data aggregations in performance-sensitive contexts. It resolves both the memory allocation overhead and the stack size vulnerability.
