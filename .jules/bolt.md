...
## 2026-03-23 - Optimized edge lookup in NodeEngine execution loop
**Learning:** In the NodeEngine execution loop, repeatedly searching for incoming edges by target node ID in the `edges` array using `Array.prototype.filter` or `Array.prototype.find` resulted in O(N * E) complexity, where N is the number of nodes in the execution path and E is the total number of edges. This was especially pronounced for `arithmetic`, `correlation`, and `dashboardOutput` nodes.
**Action:** Pre-index the `edges` array into a `Map` keyed by `target` node ID before the loop to reduce lookup time to O(1) per step, achieving an overall time complexity of O(N + E). This scales far more gracefully for large, highly connected node graphs.
