...
## 2026-07-25 - Prevent re-renders in global event listeners
**Learning:** Subscribing to an entire collection (like `useNodeStore(s => s.nodes)`) in a hook that sets up global event listeners (like keyboard shortcuts) causes the component to re-render and the listeners to be re-bound on every item change.
**Action:** For global event listeners that only need to read the current state upon interaction, use `store.getState()` or equivalent non-reactive getters inside the event handler instead of subscribing to the store in the hook body.
