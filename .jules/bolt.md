...
## 2024-05-17 - Optimize BackLinks list rendering
**Learning:** Using Object.entries on dynamic entry.data objects within list components like BackLinkItem causes significant overhead for large datasets. Additionally, looking up schemas using schemas.find(s => s._id === entry.schemaId) inside each list item creates O(N*M) complexity.
**Action:** Lift state subscriptions (Zustand/useParams) up to the parent component. Memoize arrays into a Map for O(1) lookups and pass the result down as a prop. Pre-calculate relation fields from the schema definition and only iterate over those specific keys instead of the entire data object.
