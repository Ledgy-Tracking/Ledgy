...
## 2024-05-28 - Optimized validation loop arrays lookup O(N^2)
**Learning:** Checking for node parents inside array iterations using `Array.prototype.find()` introduces O(N^2) complexity, resulting in heavy processing times on larger graph canvases.
**Action:** Index node arrays into `Map` or `Set` before doing cross-referencing lookups (like checking for orphaned items or testing circular references) to enforce O(1) loop time scale on heavily accessed paths.
