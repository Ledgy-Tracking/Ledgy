...
## 2024-06-24 - Group/Ungroup Nodes Array Iteration O(N*M) Performance issue
**Learning:** Chained array methods like `.filter().map()` inside Zustand action handlers (e.g., `groupNodes`, `ungroupNodes`) where another `Array.prototype.find()` is used per item introduces severe $O(N \times M)$ overhead for large node graphs. This blocks the main thread when grouping or ungrouping many nodes.
**Action:** Replace $O(N \times M)$ nested lookups with $O(N + M)$ pre-indexed `Map` collections and single-pass `for` loops in hot store updates.
