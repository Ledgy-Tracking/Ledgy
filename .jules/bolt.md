...
## 2024-09-13 - Optimize Bounding Box Calculation
**Learning:** For application calculating spatial boundaries (like a canvas or node editor), chaining `.map()` and passing the array into `Math.min(...arr)` or `Math.max(...arr)` can cause a `RangeError: Maximum call stack size exceeded` for large datasets and adds significant overhead due to intermediate array allocations.
**Action:** Replace multiple `.map()` passes and spread operations with a single-pass `for` loop to compute min/max boundaries directly. Keep modifications strictly focused on the performance goal to avoid rejection for out-of-scope dependency changes (e.g. replacing `nanoid`).
