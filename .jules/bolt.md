...
## 2024-05-28 - Optimize Array Lookups in Loops
**Learning:** Using `Array.prototype.find` inside a loop over another array (e.g., mapping `selectedValues` to entry data in `RelationCombobox.tsx`) creates an O(M * N) bottleneck.
**Action:** When mapping or filtering arrays based on secondary arrays of identifiers, pre-compute a lookup `Map` (keyed by identifier) for the source data array first. This converts the inner lookup to O(1), optimizing overall time complexity from O(M * N) to O(M + N).
