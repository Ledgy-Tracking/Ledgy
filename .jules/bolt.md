...
## 2024-05-18 - Math spread operator limitations
**Learning:** Spreading extremely large arrays of un-paginated ledger entries directly into `Math.min(...values)` or `Math.max(...values)` can cause "Maximum call stack size exceeded" errors due to V8 call stack limitations.
**Action:** Always replace spread operators with simple iterative O(N) `for` loops when finding max/min bounds across potentially large dynamic collections to prevent unexpected runtime crashes and enhance overall processing speed.
