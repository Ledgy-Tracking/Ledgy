...
## 2024-05-28 - Optimize child component store subscriptions
**Learning:** Having many child components (like list items) independently subscribe to a global store (e.g., `useLedgerStore()`) causes significant memory overhead and unnecessary re-renders when rendering large lists.
**Action:** When mapping over lists to render child components, avoid using global store hooks inside the child. Instead, extract the required state in the parent component (memoizing if necessary) and pass it down to the children as props.
