...
## 2024-05-28 - Optimized React Flow Viewport Handling
**Learning:** In React Flow, subscribing directly to the viewport transform via `useStore(s => s.transform)` triggers continuous, 60fps component re-renders during drag, pan, or zoom interactions. This causes severe application lag, especially when tied to computationally expensive hooks like spatial indexing.
**Action:** Replace `useStore(s => s.transform)` with `useOnViewportChange({ onEnd: callback })` to defer expensive spatial or state recalculations until the viewport interaction has completed, completely removing the main thread bottleneck during the gesture.
