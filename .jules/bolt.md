...
## 2024-05-28 - Optimize array iteration with single-pass loops
**Learning:** Combining chained array iterations (like `.map().filter()`) with spread operators (like `Math.min(...values)`) in high-frequency aggregation functions introduces significant redundant iteration, intermediate array allocations, and risk of exceeding the call stack.
**Action:** Consolidate multiple array traversal operations into a single-pass `for` loop to compute all required aggregate values (e.g., min, max, sum, count) in O(N) time with O(1) space complexity.
