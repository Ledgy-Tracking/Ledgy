...
## 2024-05-19 - Avoid Spread Operator in Math.min/max for Large Arrays
**Learning:** Using `Math.min(...arr)` or `Math.max(...arr)` where `arr` is an array created via `.map()` causes a "Maximum call stack size exceeded" error in the V8 engine if the array exceeds ~100,000 elements. Additionally, repeatedly mapping arrays creates significant garbage collection pressure due to intermediate allocations.
**Action:** Always replace spread operators into variadic mathematical functions with explicit, single-pass `for` loops that evaluate minimums and maximums inline. Ensure you check for empty arrays explicitly to avoid returning ±Infinity.
