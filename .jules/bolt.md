...
## 2024-05-28 - Optimize Math.min/max spread with custom loops
**Learning:** Using `Math.max(...array)` or `Math.min(...array)` with large arrays can cause the "Maximum call stack size exceeded" error. It also unnecessarily iterates over the array multiple times if you need both min and max, or need to compute them alongside other operations.
**Action:** Replace `Math.max(...array)` and `Math.min(...array)` with custom loops, which solves the call stack issue and can improve performance by allowing multiple computations in a single pass.
