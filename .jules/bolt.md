...
## 2024-05-28 - Optimize Zustand Array Filters
**Learning:** Using `Array.prototype.find` inside an array `filter` or `map` loop (e.g., `currentNodes.filter(currentNode => previousNodes.find(p => p.id === currentNode.id))`) within Zustand subscriptions causes severe lag during high-frequency events due to O(N^2) complexity.
**Action:** Always index previous arrays into a `Map` for O(1) lookups before filtering or iterating over current arrays in Zustand store subscriptions, reducing the complexity to O(N).
