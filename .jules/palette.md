...
## 2025-02-24 - Semantic ARIA Expanded for Layout Toggles
**Learning:** For layout toggle buttons that expand or collapse sections (such as sidebars or inspector panels), just having an `aria-label` (like "Collapse sidebar") is not enough to semantically convey the current layout state to a screen reader. The user needs to know whether the target panel is currently expanded or collapsed.
**Action:** Always dynamically bind the `aria-expanded` attribute to the boolean visibility state of the target panel (e.g., `aria-expanded={isOpen}`) on the toggle buttons to ensure the state is correctly communicated to assistive technologies.
