...
## 2025-04-25 - ARIA Expanded on Layout Toggles
**Learning:** Layout toggle buttons that show or hide sections (e.g., sidebars, inspector panels) must explicitly add and dynamically bind the `aria-expanded` attribute to the boolean visibility state of the target panel (e.g., `aria-expanded={isOpen}`) to communicate the expanded/collapsed state to screen readers.
**Action:** Always verify that buttons toggling visual regions include `aria-expanded` bound to their state, alongside appropriate `aria-label`s.
